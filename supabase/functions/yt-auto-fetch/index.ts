...
Deno.serve(async () => {
  ...
  const defaultKworbUrl = `https://kworb.net/youtube/video/${youtubeVideoId}.html`
  ...
  try {
    ...
    const { error: insertError } = await supabase.from('emoji_monitoring_snapshots').insert({
    ...
    try {
      const kworbResponse = await fetch(kworbUrl, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; YTSteamEmojiMonitor/1.0)',
        },
      })
      if (!kworbResponse.ok) {
