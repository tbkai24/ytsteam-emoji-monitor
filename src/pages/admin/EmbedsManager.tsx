...
function EmbedsManager() {
  ...
  const addRow = async () => {
    ...
    const { error: insertError } = await supabase.from('emoji_embeds').insert({
      title: finalTitle,
      url: url.trim(),
  ...
  const deleteRow = async (id: string) => {
    setSaving(true)
    setError(null)
    const { error: deleteError } = await supabase.from('emoji_embeds').delete().eq('id', id)
    if (deleteError) {
  ...
  const persistSortOrder = useCallback(async (ordered: EmbedRow[]) => {
    const updates = ordered.map((item, index) =>
      supabase.from('emoji_embeds').update({ sort_order: index }).eq('id', item.id),
    )
