...
function Settings() {
  ...
  const loadSettings = useCallback(async () => {
    ...
    if (data) {
      ...
      setWebsiteName((row.website_name ?? 'SB19 Youtube Streamers').trim() || 'SB19 Youtube Streamers')
      setLogoUrl(
        (row.logo_url ?? 'https://pbs.twimg.com/profile_images/2017991130743517184/H4hlFG2S_400x400.jpg').trim() ||
  ...
  const saveSettings = async () => {
    ...
    const payload = {
      ...
      website_name: websiteName.trim() || 'SB19 Youtube Streamers',
      logo_url:
        logoUrl.trim() || 'https://pbs.twimg.com/profile_images/2017991130743517184/H4hlFG2S_400x400.jpg',
    ...
    const { error: upsertError } = await supabase.from('emoji_app_settings').upsert(payload, { onConflict: 'id' })
    ...
    if (upsertError) {
      if (
        ...
        setError(
          'Missing emoji_app_settings columns. Add website_name, logo_url, embeds_title, embeds_subtitle, footer_text, x_url, facebook_url, instagram_url, then save again.',
        )
      } else {
          ...
          <div className="stack-sm">
            ...
            <label className="stack-xs">
              ...
              <Input
                ...
                placeholder="e.g. © 2026 SB19 Youtube Streamers. All rights reserved."
              />
            </label>
            <label className="stack-xs">
