...
function ViewsMonitoring() {
  ...
  const loadSettings = useCallback(async () => {
    ...
    const { data, error: fetchError } = await supabase.from('emoji_app_settings').select('*').eq('id', 1).maybeSingle()
  ...
  const loadRecentSnapshots = useCallback(async () => {
    ...
    const { data, error: fetchError } = await supabase
      .from('emoji_monitoring_snapshots')
    ...
    if (fetchError) {
      setError(
        `${fetchError.message}. Ensure table "emoji_monitoring_snapshots" exists with columns: captured_at, views, likes, comments, views_per_hour.`,
      )
  ...
  const saveRangeSettings = async () => {
    ...
    const { error: upsertError } = await supabase.from('emoji_app_settings').upsert(payload).eq('id', 1)
    if (upsertError) {
      if (upsertError.message.includes('enable_range_1h')) {
        setError(
          'Missing DB columns for range toggles. Run migration: add enable_range_1h, enable_range_24h, enable_range_7d to emoji_app_settings.',
        )
      } else {
  ...
  const handleInsertSnapshot = async () => {
    ...
    const { error: insertError } = await supabase.from('emoji_monitoring_snapshots').insert(payload)
    if (insertError) {
