  ...
  useEffect(() => {
    const loadStatic = async () => {
      ...
      const [settingsRes, milestonesRes] = await Promise.all([
        supabase.from('emoji_app_settings').select('*').eq('id', 1).maybeSingle(),
        supabase
          .from('emoji_milestones')
  ...
  return (
    <div className="public-stack">
      {showCongratsModal ? (
        <div className="congrats-modal-backdrop">
          <div className="congrats-modal">
            ...
            <p className="congrats-subtitle">SB19 &quot;Emoji&quot; has reached</p>
      ...
      <section className="public-hero">
        ...
        <h1>{settings?.hero_title ?? "SB19 - 'Emoji' (Official MV)"}</h1>
                ...
                <>
                  ...
                  >
                    <svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
                      {chartModel.yTicks.map((tick) => (
                        <line
                          ...
                          x2={100}
                          y2={tick.y}
                          className="public-chart-grid-line public-chart-grid-line-y"
                        />
                      ))}
                      {chartModel.xTicks.map((tick) => (
                        <line
                          ...
                          x2={tick.x}
                          y2={100}
                          className="public-chart-grid-line public-chart-grid-line-x"
                        />
                      ))}
                      <line x1={0} y1={100} x2={100} y2={100} className="public-chart-baseline" />
                      <path
                    ...
                    {hoveredChartIndex !== null ? (
                      ...
                      >
                        <span>
                          {new Date(chartModel.points[hoveredChartIndex]?.t ?? nowTick).toLocaleTimeString([], {
                            hour: 'numeric',
                            minute: '2-digit',
                          })}
                        </span>
